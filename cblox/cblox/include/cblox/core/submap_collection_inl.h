...
SubmapType SubmapCollection<SubmapType>::draftNewSubmap() const {
  const Transformation dummy_pose = Transformation();  //初始化
  const SubmapID new_submap_id = getNextSubmapID();    //初始化
  return SubmapType(dummy_pose, new_submap_id, submap_config_); //Voxgraph submap 初始化
}
