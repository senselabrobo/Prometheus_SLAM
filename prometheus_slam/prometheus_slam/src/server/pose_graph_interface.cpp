...
void PoseGraphInterface::optimize(bool enable_registration) {
  ...
  // using loop closure constraints instead
  pose_graph_.optimize(true);  
  
  //added , overlapdetection needs submapcollection's pose
  updateSubmapCollectionPoses();
...
void PoseGraphInterface::updateSubmapRPConstraints() {
  ...
  for (int cid = 0; cid < cox_submap_collection_ptr_->getClientNumber();
    ...
    for (int i = 0; i < cli_ser_sm_ids.size() - 1; i++) {
      ...
      std::cout<<"client_"<<cid<<" submap_"<< sid_i <<" pose =  "<< std::endl 
      << cox_submap_collection_ptr_->getSubmapPtr(sid_i)->getPose() <<std::endl;
      std::cout<<"client_"<<cid<<" submap_"<< sid_j <<" pose =  "<< std::endl 
      << cox_submap_collection_ptr_->getSubmapPtr(sid_j)->getPose() <<std::endl;
      std::cout<<" "<<std::endl;
