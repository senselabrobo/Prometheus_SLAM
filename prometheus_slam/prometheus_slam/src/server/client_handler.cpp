...
void ClientHandler::timeLineCallback(
    const prometheus_slam_msgs::TimeLine& time_line_msg) {
...
void ClientHandler::advertiseTopics() {
  ...
  sm_pose_tf_pub_ = nh_.advertise<prometheus_slam_msgs::MapTransform>(
...
void ClientHandler::subscribeToServices() {
  ...
  pub_client_submap_client_ = nh_.serviceClient<prometheus_slam_msgs::ClientSubmapSrv>(
      client_node_name_ + "/get_client_submap"); 

  LOG(INFO) << log_prefix_ << "Subscribed to service: "
            << client_node_name_ + "/get_all_submaps";
  get_all_submaps_client_ = nh_.serviceClient<prometheus_slam_msgs::SubmapsSrv>(
      client_node_name_ + "/get_all_submaps");

  LOG(INFO) << log_prefix_ << "Subscribed to service: "
            << client_node_name_ + "/get_pose_history";
  get_pose_history_client_ = nh_.serviceClient<prometheus_slam_msgs::PoseHistorySrv>(
      client_node_name_ + "/get_pose_history");
      
}

ClientHandler::ReqState ClientHandler::requestSubmapByTime(
    const ros::Time& timestamp, //map_fusion_msg.from_timestamp
    const SerSmId& ser_sid, 
    CliSmId* cli_sid,
    CliSm::Ptr* submap, 
    Transformation* T_Sm_C_t
    ) 
    {
  std::lock_guard<std::mutex> submap_request_lock(submap_request_mutex_);

  if (!time_line_.hasTime(timestamp)) return ReqState::FUTURE;  //Logically this is never gonna happen

  prometheus_slam_msgs::ClientSubmapSrv cli_submap_srv;
  ...
  if (pub_client_submap_client_.call(cli_submap_srv)) {
    ...
    *cli_sid = cli_submap_srv.response.submap.map_header.id;  //clientx_submap_id
    ROS_ERROR_STREAM("Client_"<<log_prefix_<<" submap_id="<<*cli_sid
                      <<" , in Server submap_id="<<ser_sid);
    *submap = utils::cliSubmapFromMsg(ser_sid, submap_config_,
                                      cli_submap_srv.response, &map_frame_id_); //generating esdf from tsdf
    ROS_ERROR_STREAM( "map_frame_id_ = "<< map_frame_id_ );
...
void ClientHandler::submapPoseUpdatesCallback(
    const prometheus_slam_msgs::MapPoseUpdates& map_pose_updates_msg) {
  ...
  for (int i = 0; i < map_pose_updates_msg.submap_id.size(); i++) {
    ...
    CHECK(submap_collection_ptr_->exists(ser_sm_id))
        ...
        << ", SerSmId: " << ser_sm_id;
    ROS_ERROR("ClientHandler::submapPoseUpdatesCallback");
...
bool ClientHandler::requestAllSubmaps(std::vector<CliSmPack>* submap_packs,
  ...
  submap_packs->clear();
  prometheus_slam_msgs::SubmapsSrv submap_srv;
...
bool ClientHandler::requestPoseHistory(const std::string& file_path,
  ...
  prometheus_slam_msgs::PoseHistorySrv pose_history_srv;
