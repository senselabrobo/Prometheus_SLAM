#ifndef PROMETHEUS_SLAM_MOD_LOOP_CLOSURE_PUBLISHER_H_
#define PROMETHEUS_SLAM_MOD_LOOP_CLOSURE_PUBLISHER_H_

#include <prometheus_slam_msgs/LoopClosure.h>
...
#include "prometheus_slam_mod/common.h"
 ...
 public:
  ...
  LoopClosurePublisher(const ros::NodeHandle& nh,
    ...
    need_to_fuse_client_ =
        nh_private_.serviceClient<prometheus_slam_msgs::NeedToFuseSrv>(
            need_to_fuse_srv_name_);

    if (client_number_ == -1) {
      loop_closure_pub_.emplace(
          -1, nh_private_.advertise<prometheus_slam_msgs::LoopClosure>(
                  "loop_closure_out", 10, true));   
    } else {
      for (int i = 0; i < client_number_; i++) {
        loop_closure_pub_.emplace(
            i, nh_private_.advertise<prometheus_slam_msgs::LoopClosure>(
  ...
  bool publishLoopClosure(size_t from_client_id, double from_timestamp,
    ...
    prometheus_slam_msgs::MapFusion map_fusion_msg;
    ...
    if (from_client_id != to_client_id) {
      ...
      if (!loop_closure_pub_.count(-1))
        loop_closure_pub_.emplace(
            -1, nh_private_.advertise<prometheus_slam_msgs::MapFusion>("map_fusion",
                                                                10, true));
  ...
  bool publishLoopClosure(CliId cid, double from_timestamp, double to_timestamp,
                          geometry_msgs::Quaternion rotation,
                          geometry_msgs::Vector3 transform) {
    prometheus_slam_msgs::LoopClosure loop_closure_msg;
  ...
  bool needToFuse(CliId cid_a, CliId cid_b, ros::Time time = ros::Time::now()) {
    prometheus_slam_msgs::NeedToFuseSrv need_to_fuse_srv;
    ...
    { ROS_ERROR("CALLING NEED TO FUSE SRV");
