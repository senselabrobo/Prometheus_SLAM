...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        odometry.header.frame_id = ORIGIN_FRAME_NAME;
        odometry.child_frame_id = ORIGIN_FRAME_NAME;
        Quaterniond tmp_Q;
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        if(STEREO)
        {
            ...
            odometry_r.header.frame_id = ORIGIN_FRAME_NAME;
            ...
            if(PUB_RECTIFY)
            {
                ...
                R_pose_l.header.frame_id = ORIGIN_FRAME_NAME;
                R_pose_r.header.frame_id = ORIGIN_FRAME_NAME;
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    br.sendTransform(tf::StampedTransform(transform, header.stamp, ORIGIN_FRAME_NAME, IMU_FRAME_NAME));
    ...
    transform.setRotation(q);
    br.sendTransform(tf::StampedTransform(transform, header.stamp, IMU_FRAME_NAME, CAMERA_FRAME_NAME));
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        point_cloud.header.frame_id = ORIGIN_FRAME_NAME;
        for (auto &it_per_id : estimator.f_manager.feature)
        {
