...
void PoseGraph::registerPub(ros::NodeHandle &n)
{
    ...
    vio_interface = new prometheus_slam::mod::VIOInterface(n, ros::NodeHandle("~"));
}
...
void PoseGraph::publishKeyFrame(KeyFrame *cur_kf) {
  ...
  cv::Mat combined_descriptor(
      cur_kf->brisk_window_descriptors_.rows + cur_kf->brisk_descriptors_.rows,
      cur_kf->brisk_window_descriptors_.cols, cur_kf->brisk_descriptors_.type());
  if (cur_kf->brisk_window_descriptors_.rows > 0) {
    cur_kf->brisk_window_descriptors_.copyTo(
        combined_descriptor.rowRange(
          0, cur_kf->brisk_window_descriptors_.rows));
  }
  if (cur_kf->brisk_descriptors_.rows > 0) {
    cur_kf->brisk_descriptors_.copyTo(combined_descriptor.rowRange(
        cur_kf->brisk_window_descriptors_.rows,
        cur_kf->brisk_window_descriptors_.rows + cur_kf->brisk_descriptors_.rows));
  }
  ...
  size_t point_idx = 0;
  for (size_t i = 0; i < cur_kf->brisk_window_keypoints.size(); ++i) {
    while (true) {
      if ((cur_kf->point_2d_uv[point_idx].x ==
           cur_kf->brisk_window_keypoints[i].pt.x) &&
          (cur_kf->point_2d_uv[point_idx].y ==
           cur_kf->brisk_window_keypoints[i].pt.y)) {
        break;
      }
      ++point_idx;
    }
    ...
    cv::Point2f tmp_point2d = cur_kf->brisk_window_keypoints[i].pt;
  ...
  for (size_t i = 0; i < cur_kf->brisk_keypoints.size(); ++i) {
    comm_msgs::keypoint tmp_kp;
    cv::Point2f tmp_point2d = cur_kf->brisk_keypoints[i].pt;
    tmp_kp.x = tmp_point2d.x;
    tmp_kp.y = tmp_point2d.y;
    keyframe_msg.keyPts.push_back(tmp_kp);
  }
