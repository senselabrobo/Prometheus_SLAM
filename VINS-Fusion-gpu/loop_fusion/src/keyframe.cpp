				   ...
				   brisk::BriskDescriptorExtractor &_brisk_extractor)
{
	...
	computeBRISKPoint(_brisk_extractor);
  	computeWindowBRISKPoint(_brisk_extractor);
	if(!DEBUG_IMAGE)
		image.release();
}
...
void KeyFrame::computeWindowBRISKPoint(
  ...
  for(int i = 0; i < (int)point_2d_uv.size(); i++)
	{
	    ...
	    key.pt = point_2d_uv[i];
      key.octave = 0;
	    brisk_window_keypoints.push_back(key);
  }
  brisk_extractor.compute(image, brisk_window_keypoints, brisk_window_descriptors_);
}

void KeyFrame::computeBRISKPoint(
    brisk::BriskDescriptorExtractor &brisk_extractor) {
  brisk_keypoints.clear(); 
  vector<cv::Point2f> tmp_pts;
  cv::goodFeaturesToTrack(image, tmp_pts, 500, 0.01, 10);
  for (size_t i = 0; i < tmp_pts.size(); ++i) {
    ...
    key.pt = tmp_pts[i];
    key.octave = 0;
    brisk_keypoints.push_back(key);
  }

  brisk_extractor.compute(image, brisk_keypoints, brisk_descriptors_);
}
