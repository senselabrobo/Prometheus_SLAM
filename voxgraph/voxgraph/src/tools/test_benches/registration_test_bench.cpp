...
int main(int argc, char** argv) {
  ...
  CHECK(submap_collection_ptr->getSubmapPose(reference_submap_id,
                                             &transform_getter));
  const Transformation T_odom__reference = transform_getter;
  CHECK(submap_collection_ptr->getSubmapPose(reading_submap_id,
                                             &transform_getter));
  const Transformation T_odom__reading_original = transform_getter;
  const Eigen::Vector3f& ground_truth_position =
      T_odom__reading_original.getPosition();

  // Publish TFs for the reference and reading submap
  TfHelper::publishTransform(T_odom__reference, "odom", "reference_submap",
                             true);
  TfHelper::publishTransform(T_odom__reading_original, "odom", "reading_submap",
                             true);
  ...
  // Publish the meshes used to visualize the submaps
  {
    ...
    TfHelper::publishTransform(T_odom__reading_original, "odom",
        ...
        for (const float& yaw : range_yaw)
          for (const float& pitch : range_pitch)
            for (const float& roll : range_roll) {
              ...
              Transformation T_odom__reading_perturbed;
              T_odom__reading_perturbed.getRotation() =
                  T_odom__reading_original.getRotation() * rotation;
              Transformation::Vector3 translation(x, y, z);
              T_odom__reading_perturbed.getPosition() =
                  T_odom__reading_original.getPosition() + translation;
              submap_collection_ptr->setSubmapPose(reading_submap_id,
                                                   T_odom__reading_perturbed);
              ...
              TfHelper::publishTransform(T_odom__reading_perturbed, "odom",
                                         "perturbed_submap", true);

              // Set initial conditions
              ceres::Solver::Summary summary;
              Transformation::Vector6 T_vec_read =
                  T_odom__reading_perturbed.log();
              double odom_pose_reading[4] = {T_vec_read[0], T_vec_read[1],
                                             T_vec_read[2], T_vec_read[5]};

              // Optimize submap registration
              bool registration_successful = submap_registerer.testRegistration(
                  reference_submap_id, reading_submap_id, odom_pose_reading,
                  &summary);
              if (registration_successful) {
                // Update reading submap pose with the optimization result
                T_vec_read[0] = odom_pose_reading[0];
                T_vec_read[1] = odom_pose_reading[1];
                T_vec_read[2] = odom_pose_reading[2];
                T_vec_read[5] = odom_pose_reading[3];
                const Transformation T_odom__reading_optimized =
                    Transformation::exp(T_vec_read);
                submap_collection_ptr->setSubmapPose(reading_submap_id,
                                                     T_odom__reading_optimized);

                // Announce results
                const Eigen::Vector3f& optimized_position =
                    T_odom__reading_optimized.getPosition();
                printf(
                    "-- % 2i remaining error:    "
                    "x % 4.6f    y % 4.6f    z % 4.6f    "
                    "yaw % 4.2f    pitch % 4.2f    roll % 4.2f    "
                    ...
                    T_odom__reading_optimized.log()[5] -
                        T_odom__reading_original.log()[5],
