...
void PoseGraphInterface::updateRegistrationConstraints() {
  ...
  updateOverlappingSubmapList();  //所有子地图都会被进行检测，包括fused和重新request的all submap
...
void PoseGraphInterface::updateSubmapCollectionPoses() {
  ...
  if( submap_collection_ptr_->getActiveSubmapID() >= 1 )
  {
    Transformation T_O_L = submap_collection_ptr_->getActiveSubmapPose();

    Transformation T_I_L ;
    pose_graph_.getSubmapPose(submap_collection_ptr_->getActiveSubmapID(), &T_I_L);

    Transformation T_O_I = T_O_L * T_I_L.inverse();

    for (const auto& submap_pose_kv : pose_graph_.getSubmapPoses()) {
      Transformation T_O_S_optimized;
      T_O_S_optimized = T_O_I * submap_pose_kv.second ;
