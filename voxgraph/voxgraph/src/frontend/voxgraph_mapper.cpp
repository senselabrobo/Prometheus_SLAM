...
void VoxgraphMapper::getParametersFromRos() {
  ...
  std::cout<< "odom_frame = " << frame_names_.output_odom_frame << std::endl;
...
void VoxgraphMapper::loopClosureCallback(
  ...
  ROS_ERROR("WE ARE IN VoxgraphMapper::loopClosureCallback");
...
bool VoxgraphMapper::submapCallback(
  ...
  VoxgraphSubmap new_submap = draftNewSubmap();  //析构的是这个函数，submap初始化
  ...
  const Transformation T_odom_submap =
      VoxgraphSubmapCollection::gravityAlignPose(
          T_odom_trajectory_middle_pose.cast<voxblox::FloatingPoint>()); //T_O_S

  // NOTE: Generating ESDF from TSDF for submap with ID
  // set submap pose to the middle pose of the trajectory
  ...
  addSubmap(std::make_shared<VoxgraphSubmap>(new_submap));  //NOTE: Created submap x with pose 
  ...
  pose_graph_interface_.addSubmap(active_submap_id); //NOTE: Added node to graph for submap: x
  ...
  if (submap_collection_ptr_->size() > 1) {
    ...
    const Transformation T_previous_submap__submap =
        T_odom__previous_submap.inverse() * T_odom_submap;  // T_S1_S2
...
int VoxgraphMapper::optimizePoseGraph() {
  ...
  ROS_ERROR("Finished Optimizing !!!!!!!!!");
  ROS_WARN_STREAM("optimized submap_" << submap_collection_ptr_->getActiveSubmapID() 
                  <<" pose ="<<std::endl
                  << submap_collection_ptr_->getActiveSubmapPtr()->getPose());
